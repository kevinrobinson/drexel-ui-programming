...
//
// For demo purposes, we'll seed this here by reading some data stored
// on disk.
var allSongsOnServer = JSON.parse(fs.readFileSync('data/seed_songs.json'));
...
// Handle `POST` requests where a user stars a specific song.
app.post('/songs/:id/star', function(req, res){
	// Read the request parameters.  In this case, we grab the
	// `id` from the request path so we can use it
	// to lookup the `song` that the request is talking about.
	var songId = req.params.id;

	// Find the song in the server's memory.
	var song = allSongsOnServer[songId];
	...
	// Add another star to the song.  This mutates the song in the
	// server's memory, so that subsequent GET requests will show
	// the new number.  In a production system, this would be done
	// with a database or other storage system.
	if (!song.stars) {
		song.stars = 0;
	}
	song.stars++;
